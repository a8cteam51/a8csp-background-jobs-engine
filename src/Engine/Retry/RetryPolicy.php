...
/**
 * Defines bounded exponential retry delays with full jitter.
 *
 * @since   1.0.0
 * @version 1.0.0
 */
final readonly class RetryPolicy {
	...
	/**
	 * Returns the full-jitter delay separating a failed attempt from its next attempt.
	 *
	 * Attempt one produces the first retry and draws uniformly from `[0, base_delay]`.
	 *
	 * @since   1.0.0
	 * @version 1.0.0
	 *
	 * @param   int                 $attempt    One-indexed number of the just-failed attempt.
	 * @param   RandomizerInterface $randomizer Integer randomness source.
	 *
	 * @return  int
	 *
	 * @throws  \InvalidArgumentException When the attempt has no following attempt.
	 */
	public function delay_for_attempt( int $attempt, RandomizerInterface $randomizer ): int {
		...
		return $randomizer->int( 0, $ceiling );
	}
