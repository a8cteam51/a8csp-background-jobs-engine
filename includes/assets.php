 ...
 * @return  array|null
 */
function a8csp_background_tasks_get_asset_meta( string $asset_path, ?array $extra_dependencies = null ): ?array {
	if ( ! file_exists( $asset_path ) || ! str_starts_with( $asset_path, A8CSP_BACKGROUND_TASKS_PATH ) ) {
		return null;
	}
	...
	if ( file_exists( $asset_path_info['dirname'] . '/' . $asset_path_info['filename'] . '.asset.php' ) ) {
		...
		if ( false === $asset_meta['version'] ) { // Safeguard against filemtime() returning false.
			$asset_meta['version'] = A8CSP_BACKGROUND_TASKS_METADATA['Version'];
		}
	}
