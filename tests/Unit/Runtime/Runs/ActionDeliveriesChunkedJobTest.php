...
final class ActionDeliveriesChunkedJobTest extends TestCase {
	...
	public function test_handle_continue_action_schedules_cleanup_for_an_empty_queue(): void {
		...
		self::assertCount( 1, $this->calls_for_hook( 'a8csp_jobs_engine/cleanup_chunked_job' ) );
	}
	...
	public function test_handle_continue_action_resolves_continue_delay_filter_values( mixed $filtered_delay, int $expected_delay ): void {
		...
		self::assertSame( self::NOW + 120 + $expected_delay, $this->single_call_for_hook( 'a8csp_jobs_engine/continue_chunked_job' )['args']['timestamp'] ?? null );
	}
	...
	public function test_handle_continue_action_supersedes_when_continue_delay_filter_loses_ownership(): void {
		...
		$this->assert_foreign_superseded();
		self::assertSame( array(), $this->calls_for_hook( 'a8csp_jobs_engine/continue_chunked_job' ) );
	}
	...
	public function test_duplicate_cleanup_delivery_fires_on_completed_once(): void {
		...
		$this->rig->run_due();
		\do_action( 'a8csp_jobs_engine/cleanup_chunked_job', self::IDENTITY, self::RUN_ID, $sequence );
