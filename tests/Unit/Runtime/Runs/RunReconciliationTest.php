...
final class RunReconciliationTest extends TestCase {
	...
	public function test_sweep_redelivers_a_stale_pending_chunked_job_continue_and_processes_its_chunk(): void {
		...
		$identity           = self::boundary_identity( $name );
		$this->registry->register( $identity, $chunked_job->definition() );
		$result = $this->dispatcher->dispatch( $identity, self::ARGS );
	...
	public function test_sweep_redelivers_a_stale_pending_chunked_job_start_for_every_overlap_policy( string $overlap_value ): void {
		...
		$identity    = self::boundary_identity( $name );
		$this->registry->register( $identity, $chunked_job->definition( new JobOptions( overlap: OverlapPolicy::from( $overlap_value ) ) ) );
		$result = $this->dispatcher->dispatch( $identity, self::ARGS, priority: 23 );
		self::assertInstanceOf( Success::class, $result );
	...
	public function test_sweep_redelivers_a_stale_pending_job_action(): void {
		$result = $this->dispatcher->dispatch_scheduled_target( $this->identity, self::ARGS, priority: 23 );
		self::assertInstanceOf( Success::class, $result );
		$this->backend->calls   = array();
		$this->clock->timestamp = self::NOW + 901;
	...
	/**
	 * A terminalized scheduled-target resolver failure retains its priority for manual retry.
	 *
	 * @return  void
	 */
	public function test_overlap_key_failure_retains_scheduled_target_priority_for_retry(): void {
		$name     = self::identity( 'resolver-priority' );
		$identity = self::boundary_identity( $name );
		$resolver = new class() {
			...
			public function resolve(): string {
				if ( $this->fails ) {
					throw new \RuntimeException( 'Consumer resolver failed.' );
				}
		...
		$job      = new RecordingJob( 'resolver-priority' );
		$this->registry->register(
			$identity,
			$job->definition(
				new JobOptions(
					overlap_key: static fn ( array $args ): string => $resolver->resolve()
				)
			)
		);

		$failed = $this->dispatcher->dispatch_scheduled_target( $identity, self::ARGS, priority: 42, terminalize_overlap_key_failure: true );

		self::assertInstanceOf( Success::class, $failed );
		self::assertIsString( $failed->value );
		$retained = $this->options()[ FailedRunStore::OPTION_PREFIX . $name ] ?? null;
		...
		$retried = $this->dispatcher->retry_failed( $identity, $failed->value );

		self::assertInstanceOf( Success::class, $retried );
		$deliveries = \array_values( \array_filter( $this->backend->calls, static fn ( array $call ): bool => 'enqueue_async' === $call['verb'] ) );
		self::assertCount( 1, $deliveries );
		self::assertSame(
			array( 42, 42 ),
			array( $retained_priority, $deliveries[0]['args']['priority'] ?? null )
		);
	}
	...
	public function test_pending_job_redelivery_ignores_a_current_chunked_job_with_the_same_identity(): void {
		...
		$this->registry->register( self::boundary_identity( $name ), $current_chunked_job->definition() );
	...
	public function test_sweep_continues_after_one_run_staleness_filter_throws(): void {
		...
		$healthy_identity    = self::boundary_identity( $healthy_name );
		$this->registry->register( $healthy_identity, $healthy_chunked_job->definition() );
		$result = $this->dispatcher->dispatch( $healthy_identity, self::ARGS );
		self::assertInstanceOf( Success::class, $result );
		self::assertSame( self::RUN_ID, $result->value );
	...
	public function test_sweep_continues_after_a_failed_hook_throws_for_one_chunked_job(): void {
		...
		$throwing_identity    = self::boundary_identity( $throwing_name );
		$this->registry->register( $throwing_identity, $throwing_chunked_job->definition() );
		$result = $this->dispatcher->dispatch( $throwing_identity, self::ARGS );
		self::assertInstanceOf( Success::class, $result );
		self::assertSame( self::RUN_ID, $result->value );
	...
	public function test_sweep_replays_all_effects_for_an_old_failed_chunked_job(): void {
		...
		$this->registry->register( self::boundary_identity( $name ), $chunked_job->definition() );
	...
	public function test_sweep_classifies_missing_persisted_failure_detail_as_storage_failure(): void {
		...
		$this->registry->register( self::boundary_identity( $name ), $chunked_job->definition() );
		$this->store_terminal_run( $name, 'failed', array(), null, 2, 'chunked_job', array( $chunk ), PendingAction::async( 'continue', 42 ) );
	...
	public function test_sweep_replays_only_missing_failed_chunked_job_effects(): void {
		...
		$identity    = self::boundary_identity( $name );
		$this->registry->register( $identity, $chunked_job->definition() );
		self::assertTrue( $this->stores->failed_run_store( $identity )->record( self::RUN_ID, 'chunked_job', self::NOW - 3_601, self::ARGS, 10, 2, new EngineError( 'Persisted chunked job failure.', \RuntimeException::class ), new RunFailure( identity: $name, run_id: RunId::from( self::RUN_ID ), attempts: 2, stage: RunFailureStage::execution(), code: ErrorCode::ExecutionFailed, summary: 'Persisted chunked job failure.', details: null, ) ) );
	...
	public function test_sweep_replays_only_unmarked_failed_job_hooks(): void {
		...
		$marked_identity = self::boundary_identity( $marked_name );
		$this->registry->register( $marked_identity, $marked_job->definition() );
		...
		self::assertTrue( $this->stores->failed_run_store( $marked_identity )->record( self::RUN_ID, 'job', self::NOW - 3_601, self::ARGS, 10, 2, new EngineError( 'Persisted job failure.', \RuntimeException::class ), $failure ) );
	...
	public function test_sweep_retries_a_throwing_failed_job_hook(): void {
		...
		self::assertTrue( $this->stores->failed_run_store( $this->identity )->record( self::RUN_ID, 'job', self::NOW - 3_601, self::ARGS, 10, 2, new EngineError( 'Persisted job failure.', \RuntimeException::class ), $failure ) );
	...
	public function test_terminal_finish_is_gated_until_the_sweep_completes_missing_effects(): void {
		...
		self::assertFalse( $this->terminal_effects->finish_claimed_transition( $this->identity, self::RUN_ID, $state, $snapshot['raw'], $run_store ) );
		self::assertArrayHasKey( $this->run_option_name(), $this->options() );
	 ...
	 * @return  Identity
	 */
	private static function boundary_identity( string $identity ): Identity {
		return Identity::tryFrom( $identity )
			?? throw new \LogicException( 'The test identity must satisfy the canonical grammar.' );
	}
