...
$a8csp_bgte_migration_controller = \Action_Scheduler\Migration\Controller::instance();
if ( ! $a8csp_bgte_migration_controller instanceof \Action_Scheduler\Migration\Controller ) {
...
if ( ! $a8csp_bgte_migration_controller->allow_migration() ) {
	exit( 0 );
}

$a8csp_bgte_migration_config = $a8csp_bgte_migration_controller->get_migration_config_object();
if ( ! $a8csp_bgte_migration_config instanceof \Action_Scheduler\Migration\Config ) {
	// phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fwrite -- STDERR diagnostics in a CLI-only script; WP_Filesystem targets real files.
...
$a8csp_bgte_migration_runner = new \Action_Scheduler\Migration\Runner( $a8csp_bgte_migration_config );
$a8csp_bgte_migration_runner->init_destination();
do {
	$a8csp_bgte_migrated = $a8csp_bgte_migration_runner->run( 100 );
} while ( 0 < $a8csp_bgte_migrated );
