...
final class OverlapLockTest extends AbstractIntegrationTestCase {
	...
	public function test_reject_policy_refuses_a_fresh_held_lock(): void {
		...
		self::assertSame( \sprintf( 'chunked_job "%1$s" overlap lock is held by run "%2$s"; wait for that lock to be released before dispatching the same arguments or overlap key.', self::REJECT_IDENTITY, $run_a ), $result->get_error_message(), 'The rejected held-lock failure must identify the incumbent run exactly' );
