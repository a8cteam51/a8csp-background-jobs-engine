...
Every targeted `<identity>` argument is a composed `{owner}:{name}` identity. Failed-run and schedule list output include an `owner` column, carry the composed identity in an `identity` column, and accept an exact `--owner` filter. Failed-run rows report `owner`, `identity`, `run_id`, `failed_at`, `attempts`, `stage`, `code`, `error_class`, and `error_message`; JSON and YAML additionally expose `failed_chunk`, which is `null` for task or non-chunk failures. Unreadable failed-run entries or whole option rows remain excluded while one count warning is emitted on STDERR, leaving machine-readable STDOUT parseable. The list commands accept `table`, `csv`, `json`, `count`, or `yaml`; the default is `table`. Examples matching the command help are:
